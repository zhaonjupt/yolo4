...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, label_smoothing=0.1, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        ciou_loss = K.sum(ciou_loss) / K.maximum(mf, K.epsilon())
        ...
        confidence_loss = K.sum(confidence_loss) / K.maximum(mf, K.epsilon())
        class_loss = K.sum(class_loss) / K.maximum(mf, K.epsilon())
