...
def fit_one_epoch(net, yolo_loss, optimizer, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, anchors, 
    ...
    print('*' * 30)
    with open('./logs/finetune.txt', 'w') as f:
        f.write('Epoch%d-Total_Loss%.4f-Val_Loss%.4f.h5' % ((epoch+1),loss/(epoch_size+1),val_loss/(epoch_size_val+1)))
    net.save('logs/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.h5'%((epoch+1),loss/(epoch_size+1),val_loss/(epoch_size_val+1)))
    print("mdoel and txt file have been saved !!!")
...
if __name__ == "__main__":
    ...
    #------------------------------------------------------#
    if finetune_flag:
        with open('./logs/finetune.txt') as f:
           weights_path = './logs/' + f.readline()
    else:
    ...
    Freeze_epoch = 25
    if finetune_flag:
        Init_epoch = int(re.findall("(?<=Epoch)\d*", weights_path)[0])
    else:
        Init_epoch = 0
    print('The Init_Epoch is %d' % (Init_epoch))
    ...
    if True:
        if Init_epoch < Freeze_epoch:
            ...
            if Use_Data_Loader:     #tf.data pipline
            ...
            print('Frezze train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
            ...
            print("The first 302 layers has been unfrezzed!!!")
